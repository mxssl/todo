...
func (h *handlers) DestroyOneHandler(params todos.DestroyOneParams) middleware.Responder {
	err := h.itemStore.DeleteItemByID(params.ID)
	if err == store.ErrNothingToDelete {
		...
		log.Printf("cannot delete item %d: %v", params.ID, err)
		return todos.NewFindTodosDefault(500).WithPayload(&models.Error{
			Code:    500,
			Message: swag.String("cannot delete item"),
		})
	}
